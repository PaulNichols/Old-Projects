...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Request.QueryString("docid") = "34" Then 'C4
            With CType(Session("CurrentContact"), ASPNET.StarterKit.Portal.ContactDetails)
                doc.Envelope.Insert(ExtractAddress:=False, OmitReturnAddress _
                                        :=False, PrintBarCode:=False, Address:= _
                                        .ContactAddress, Height:=WordApplication.CentimetersToPoints _
                                        (22.9), Width:=WordApplication.CentimetersToPoints(32.4), ReturnAddress:="", AddressFromLeft:=0, AddressFromTop:= _
                                        0, ReturnAddressFromLeft:=0, _
                                        ReturnAddressFromTop:=0, DefaultOrientation:=3 _
                                        , DefaultFaceUp:=True)
            End With
        ElseIf Request.QueryString("docid") = "36" Then 'C5
            With CType(Session("CurrentContact"), ASPNET.StarterKit.Portal.ContactDetails)
                doc.Envelope.Insert(ExtractAddress:=False, OmitReturnAddress _
                                        :=False, PrintBarCode:=False, Address:= _
                                        .ContactAddress, ReturnAddress:="", AddressFromLeft:=0, AddressFromTop:= _
                                        0, Height:=WordApplication.CentimetersToPoints _
                                        (16.2), Width:=WordApplication.CentimetersToPoints(22.9), ReturnAddressFromLeft:=0, _
                                        ReturnAddressFromTop:=0, DefaultOrientation:=3 _
                                        , DefaultFaceUp:=True)
