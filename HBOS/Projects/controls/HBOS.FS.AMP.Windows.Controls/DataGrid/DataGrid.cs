...
//		}
		...
		**********************************/
		...
		/// <summary>
		/// Resets the offset, this has been added to force a reset of the private member HorizontalOffset.
		/// UPD hae experienced a big red cross appearing when the grid is scrolled to the right with a boolean column and a rebind occurs
		/// </summary>
		private void ResetOffset()
		{
			if (TableStyles.Count>0)
			{
				DataGridTableStyle[] newStyles=new DataGridTableStyle[TableStyles.Count];
	
				for (int i = 0; i < TableStyles.Count; i++)
				{
					newStyles[i]=TableStyles[i];
				}
				TableStyles.Clear();
				TableStyles.AddRange(newStyles);
			}
			else
			{
				TableStyles.Clear();
