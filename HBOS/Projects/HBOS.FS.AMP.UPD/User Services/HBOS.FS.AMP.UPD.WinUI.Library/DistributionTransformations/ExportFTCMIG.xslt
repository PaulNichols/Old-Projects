<xsl:stylesheet version="1.0" 
    ...
    <xsl:template name="Header">
        ...
        <!--Comment Indicator, File Contents, File Created Title -->
    ...
    <xsl:template match="ReleasedFunds">
		...
		<xsl:call-template name='padString'>
			...
			<xsl:with-param name='value' 	select='MEXXCode'/>
		</xsl:call-template>
 ...
 <!-- Render the footer row-->
	<!--
		...
		Field Name			Value				Format		Length
		===========================================================
		FT Code				EIPP Mex code		X(6)		6
		Filler				Space				X(1)		1
		Filler				"<<"				X(2)		2
		Description	Text	"EIPP Dealing 
								Date"			X(17)		17
		Filler				">>"				X(2)		2
		Filler				Space				X(1)		1
		Filler				"<<"				X(2)		2
		Dealing Date		Valuation 
							distribution date 
							in dd/mm/ccyy 
							format				X(10)		10
		Filler				">>"				X(2)		2
    ...
    <xsl:template name="Footer">
		...
		<xsl:text>%AZZYU</xsl:text>
		<xsl:value-of select='$blank' />
		<xsl:value-of select='$LessThan' />
		<xsl:text>EIPP Dealing Date</xsl:text>
		<xsl:value-of select='$GreaterThan' />
		<xsl:value-of select='$blank' />
		
		<!-- Valuation Date -->
		<xsl:value-of select='$LessThan' />
		<xsl:value-of select="user:returnDate2()" />		
		<xsl:value-of select='$GreaterThan' />
