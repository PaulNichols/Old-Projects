...
{
	// create the template
	using T = HBOS.FS.AMP.UPD.Types.Factors.Factor;

	//
	// A collection of factors all of which have base class Factor (of which is the type for the collection)
	// and should implement IFactor. Note that Factor itself is an abstract class.
	//
	...
	/// </summary>
	[Serializable]
	public class FactorCollection : ICollection, IList, ITypedList, IEnumerable, ICloneable
	{
		...
		/// </summary>
		public FactorCollection(FactorCollection collection)
		{ 
			AddRange(collection); 
		}
		...
		/// Fund collection overloaded constructor
		/// </summary>
		public FactorCollection(T[] array)
		{ 
			AddRange(array); 
		}

		#endregion FactorCollection
		...
		{
			FactorCollection tc = new FactorCollection();
			tc.AddRange(this);
		...
		public void AddRange(FactorCollection collection)
		{
		...
		*/
		...
		{
			private FactorCollection m_collection;
			private int m_index;
			private int m_version;

			// Construction

			internal Enumerator(FactorCollection tc)
			{
				m_collection = tc;
				m_index = -1;
		...
		#region Factor specific member variables

		Hashtable m_holidays = null;

		#endregion

		#region Factor Specific Methods
		...
		public Hashtable Holidays
		{
			...
			set
			{
				...
				{
					if (this[i] is RevaluationFactor)
					{
						((RevaluationFactor)this[i]).Holidays = m_holidays;
						break;
					}
				}
			} 
		...
		/// to the collection
		/// </summary>
		public RevaluationFactor RevaluationFctr
		{
			get
			{
				RevaluationFactor revalFactor = null;

				//for with a count is more efficient than foreach
				for (int i = 0; i < this.Count; i++)
				{
					if (this[i] is RevaluationFactor)
					{
						revalFactor = (RevaluationFactor)this[i];
						break;
					}
				}
				if (revalFactor == null)
				{
					//we haven't got a revaluation factor, so create one!
					revalFactor = new RevaluationFactor();
					this.Add (revalFactor);
					revalFactor.IsNew = true;
				}
				if (m_holidays != null)
				{
					revalFactor.Holidays = m_holidays;
				}
				return revalFactor;
			}
		}
		...
		/// to the collection
		/// </summary>
		public ScalingFactor ScalingFctr
		{
			get
			{
				//for with a count is more efficient than foreach
				for (int i = 0; i < this.Count; i++)
				{
					if (this[i] is ScalingFactor)
					{
						return (ScalingFactor)this[i];
					}
				}
				//we haven't got a revaluation factor, so create one!
				ScalingFactor scalingFactor = new ScalingFactor();
				this.Add (scalingFactor);
				scalingFactor.IsNew = true;
		...
		public XFactor XFctr
		{
			get
			{
				...
				for (int i = 0; i < this.Count; i++)
				{
					if (this[i] is XFactor)
					{
						return (XFactor)this[i];
					}
				}
				//we haven't got a XFactor, so create one!
				XFactor xFact = new XFactor();
				this.Add (xFact);
				xFact.IsNew = true;
				return xFact;
			}
        ...
        /// to the collection
        /// </summary>
        public ValuationBasis ValuationBasisFctr
        {
            get
            {
                //for with a count is more efficient than foreach
                for (int i = 0; i < this.Count; i++)
                {
                    if (this[i] is ValuationBasis)
                    {
                        return (ValuationBasis)this[i];
                    }
                }
                //we haven't got a  valuation basis, so create one!
                ValuationBasis vBasis = new ValuationBasis();
                this.Add (vBasis);
                vBasis.IsNew = true;
                return vBasis;
            }
		...
		/// to the collection
		/// </summary>
		public TaxProvisionEstimate TPE
		{
			get
			{
				//for with a count is more efficient than foreach
				for (int i = 0; i < this.Count; i++)
				{
					if (this[i] is TaxProvisionEstimate)
					{
						return (TaxProvisionEstimate)this[i];
					}
				}
				//we haven't got a revaluation factor, so create one!
				TaxProvisionEstimate tpe = new TaxProvisionEstimate();
				this.Add (tpe);
				tpe.IsNew = true;
				return tpe;
			}
		...
		/// </summary>
		/// <returns></returns>
		public decimal SumFactors()
		{
			decimal factorTotal = 0M;
			for (int i = 0; i < this.Count; i++)
			{
				IFactor fctr = (IFactor)this[i];
				if (fctr.IsIncludedInSummation())
				{
					try
					{
						factorTotal += fctr.CalculateEffect();
					}
					catch 
					{
					}
