...
namespace HBOS.FS.AMP.UPD.Types.AssetFunds
{
    ...
    // Create the template
    using T = HBOS.FS.AMP.UPD.Types.AssetFunds.StockMarket;
    
    /// <summary>
	/// Summary description for MarketIndexCollection.
	/// </summary>
    [Serializable]
    public class StockMarketCollection  : ICollection, IList, ITypedList, IEnumerable, ICloneable
	{
		#region Private variables

		private const int DefaultMinimumCapacity = 1;

		private StockMarket[] m_array = new StockMarket[DefaultMinimumCapacity];
		private int m_count = 0;        // permits thread safe operations
		[NonSerialized] private int m_version = 0;

		#endregion Private variables

		#region MarketIndexCollection

		/// <summary>
		/// Fund collection constructor
		/// </summary>
		public StockMarketCollection()
		{ }

		/// <summary>
		/// Fund collection overloaded constructor
		/// </summary>
		public StockMarketCollection(StockMarketCollection collection)
		{ 
			AddRange(collection); 
		}
		...
		/// Fund collection overloaded constructor
		/// </summary>
		public StockMarketCollection(StockMarket[] array)
		{ 
			AddRange(array); 
		}

		#endregion MarketIndexCollection
		...
		public string GetListName( PropertyDescriptor[] listAccessors )
		{
			...
			{
				return typeof( StockMarket ).ToString();
			}
			else
			{
		...
		{
			return TypeDescriptor.GetProperties( typeof( StockMarket ));
		}
		...
		public void CopyTo(StockMarket[] array)
		{
			this.CopyTo(array, 0);
		}
		...
		public void CopyTo(StockMarket[] array, int start)
		{
		...
		/// </summary>
		public StockMarket this[int index]
		{
			get
			{
		...
		/// <returns></returns>
		public int Add(StockMarket item)
		{
			if (NeedsGrowth())
				Grow();
		...
		{
			++m_version;
			m_array = new StockMarket[DefaultMinimumCapacity];
			m_count = 0;
		}
		...
		/// <param name="item"></param>
		/// <returns></returns>
		public bool Contains(StockMarket item)
		{
		...
		/// <param name="item"></param>
		/// <returns></returns>
		public int IndexOf(StockMarket item)
		{
		...
		/// <param name="item"></param>
		public void Insert(int position, StockMarket item)
		{
		...
		/// </summary>
		/// <param name="item"></param>
		public void Remove(StockMarket item)
		{
		...
		/// </summary>
		/// <returns></returns>
		public StockMarketCollection Clone()
		{
			StockMarketCollection tc = new StockMarketCollection();
			tc.AddRange(this);
		...
		public int Capacity
		{
			...
			set
			{
				...
				StockMarket[] temp = new StockMarket[value];
		...
		public void AddRange(StockMarketCollection collection)
		{
		...
		*/
		...
		object IList.this[int index]
		{
			...
			set
			{ this[index] = (StockMarket)value; }
		}

		int IList.Add(object item)
		{
			return this.Add((StockMarket)item);
		}
		...
		*/
		...
		{
			return this.Contains((StockMarket)item);
		}
		...
		{
			this.Insert(position, (StockMarket)item);
		}
		...
		*/
		...
		{
			private StockMarketCollection m_collection;
			private int m_index;
			private int m_version;

			// Construction

			internal Enumerator(StockMarketCollection tc)
			{
				m_collection = tc;
				m_index = -1;
