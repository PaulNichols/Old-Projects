...
{
	using T = HBOS.FS.AMP.UPD.Types.FundGroups.FundGroup;

	/// <summary>
	/// Use this class to convert a FundGroupCollection to a SimpleLookupCollection
	/// </summary>
	public class FundGroupCollectionConverter:System.ComponentModel.TypeConverter
		{
		...
		{
			if(destinationType == typeof(SimpleLookupCollection))
			{
				return true;
			}
		...
		public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
		{
			if (destinationType == typeof(SimpleLookupCollection))
			{
				if (value is FundGroupCollection)
				{
					SimpleLookupCollection returnSimpleCollection=new SimpleLookupCollection();
					foreach (FundGroup fundGroup in (FundGroupCollection)value)
					{
						returnSimpleCollection.Add(new SimpleLookup(fundGroup.ID,fundGroup.FullName)) ;
					}
					return returnSimpleCollection;
				}
			}
	...
	/// </summary>	
	[Serializable]
	public class FundGroupCollection : ICollection, IList, ITypedList, IEnumerable, ICloneable
	{
		...
		#region FundGroupCollection

		/// <summary>
		/// FundGroup collection constructor
		/// </summary>
		public FundGroupCollection()
		{ }

		/// <summary>
		/// FundGroup collection overloaded constructor
		/// </summary>
		public FundGroupCollection(FundGroupCollection collection)
		{ 
			AddRange(collection); 
		}
		...
		/// FundGroup collection overloaded constructor
		/// </summary>
		public FundGroupCollection(T[] array)
		{ 
			AddRange(array); 
		}

		#endregion FundGroupCollection
		...
		/// </summary>
		/// <returns></returns>
		public FundGroupCollection Clone()
		{
			FundGroupCollection tc = new FundGroupCollection();
			tc.AddRange(this);
		...
		public void AddRange(FundGroupCollection collection)
		{
		...
		*/
		...
		{
			private FundGroupCollection m_collection;
			private int m_index;
			private int m_version;

			// Construction

			internal Enumerator(FundGroupCollection tc)
			{
				m_collection = tc;
				m_index = -1;
		...
		/// <returns></returns>
		public static explicit operator SimpleLookupCollection(FundGroupCollection objectToConvert)
		{
			SimpleLookupCollection returnSimpleCollection=new SimpleLookupCollection();

			if (objectToConvert != null)
			{
				foreach (FundGroup fundGroup in (FundGroupCollection)objectToConvert)
				{
					returnSimpleCollection.Add(new SimpleLookup(fundGroup.ID,fundGroup.FullName)) ;
				}
			}
