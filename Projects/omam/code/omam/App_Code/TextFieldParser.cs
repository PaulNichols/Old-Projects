...
public class TextFieldParser
{
    ...
    public TextFieldParser(string fileName)
        : this(fileName, FileFormat.FixedWidth)
    {
    }
        ...
        if (actualLineNumber >= currentLineNumber)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    throw new Exception("The number of fields identified do not match the expected number.");
                }
            }
            catch (Exception ex)
            {
    ...
    public void ParseFileContents(Stream stream)
    {
        const int BUFFER_SIZE = 255;
        int nBytesRead = 0;
        Byte[] Buffer = new Byte[BUFFER_SIZE];
        StringBuilder uploadedContent = new StringBuilder("");

        nBytesRead = stream.Read(Buffer, 0, BUFFER_SIZE);

        while (0 != nBytesRead)
        {
            uploadedContent.Append(Encoding.ASCII.GetString(Buffer, 0, nBytesRead));
            nBytesRead = stream.Read(Buffer, 0, BUFFER_SIZE);
        }


        ParseFileContents(uploadedContent.ToString());
    }
    ...
    /// <param name="objectToPopulate">The object to populate.</param>
    /// <param name="fields">The fields.</param>
    public static void FillObject(esEntity objectToPopulate, TextFieldCollection fields)
    {
...
public class TextField
{
    ...
    public TextField(string Name, TypeCode DataType, bool Quoted)
        : this(Name, DataType, 0XFFF, Quoted)
    {
        //  this constructor would be used with delimited fields
    }
    ...
    public TextField(string Name, TypeCode DataType, Int32 Length)
    ...
    /// <param name="Length">The length.</param>
    /// <param name="Quoted">if set to <c>true</c> [quoted].</param>
    public TextField(string Name, TypeCode DataType, Int32 Length, bool Quoted)
    {
    ...
    public object Value
    {
        ...
        set
        {
            try
            {
                ...
                {
                    fieldValue =
                        DateTime.ParseExact(value.ToString(), DateTimeFormat, Thread.CurrentThread.CurrentCulture);
                }
                else
                {
